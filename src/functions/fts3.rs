...
impl ExprTestNode {
    fn to_string_repr(&self) -> String {
        match self {
            ...
            ExprTestNode::And(l, r) => {
                format!("AND {{{}}} {{{}}}", l.to_string_repr(), r.to_string_repr())
            }
            ExprTestNode::Or(l, r) => {
                format!("OR {{{}}} {{{}}}", l.to_string_repr(), r.to_string_repr())
            }
            ExprTestNode::Not(l, r) => {
                format!("NOT {{{}}} {{{}}}", l.to_string_repr(), r.to_string_repr())
            }
            ExprTestNode::Near(d, l, r) => format!(
                "NEAR/{} {{{}}} {{{}}}",
                d,
                l.to_string_repr(),
                r.to_string_repr()
...
fn tokenize_exprtest(expr: &str) -> Vec<ExprTestToken> {
    ...
    while i < len {
        ...
        if ch == '(' {
            tokens.push(ExprTestToken::LParen);
            i += 1;
            continue;
        }
        if ch == ')' {
            tokens.push(ExprTestToken::RParen);
            i += 1;
            continue;
        }
        if ch == '"' {
            ...
            while i < len && chars[i] != '"' {
                ...
                if c.is_ascii_alphanumeric() || c == '_' || (!c.is_ascii() && c != '"') {
                    cw.push(c.to_ascii_lowercase());
                    i += 1;
                } else if c == '*' && !cw.is_empty() {
                    phrase_terms.push((cw.clone(), true));
                    cw.clear();
                    i += 1;
                } else {
                    if !cw.is_empty() {
                        phrase_terms.push((cw.clone(), false));
                        cw.clear();
                    }
                    i += 1;
                }
            ...
            if !cw.is_empty() {
                phrase_terms.push((cw, false));
            }
            if i < len && chars[i] == '"' {
                i += 1;
            }
            if !phrase_terms.is_empty() {
                tokens.push(ExprTestToken::QuotedPhrase(phrase_terms));
            }
            continue;
        }
        if ch.is_ascii_alphanumeric() || ch == '_' || !ch.is_ascii() {
            ...
            while i < len
                && (chars[i].is_ascii_alphanumeric() || chars[i] == '_' || !chars[i].is_ascii())
            {
                word.push(chars[i]);
                i += 1;
            }
            ...
            if upper == "AND" {
                tokens.push(ExprTestToken::And);
                continue;
            }
            if upper == "OR" {
                tokens.push(ExprTestToken::Or);
                continue;
            }
            if upper == "NOT" {
                tokens.push(ExprTestToken::Not);
                continue;
            }
            if upper == "NEAR" {
            ...
            tokens.push(ExprTestToken::Word(
                col_prefix,
...
struct ExprTestParser {
    tokens: Vec<ExprTestToken>,
    pos: usize,
    columns: Vec<String>,
    n_col: i32,
}
...
impl ExprTestParser {
    ...
    fn peek(&self) -> Option<&ExprTestToken> {
    ...
    fn resolve_column(&self, name: &str) -> i32 {
        ...
        for (idx, col) in self.columns.iter().enumerate() {
            if col.to_ascii_lowercase() == lower {
                return idx as i32;
            }
        }
        self.n_col
    }
    fn parse(&mut self) -> Result<Option<ExprTestNode>> {
        self.parse_or()
    }
    ...
    fn parse_not(&mut self) -> Result<Option<ExprTestNode>> {
        let mut left = match self.parse_near()? {
            Some(n) => n,
            None => return Ok(None),
        };
    ...
    fn parse_primary(&mut self) -> Result<Option<ExprTestNode>> {
        match self.peek().cloned() {
            Some(ExprTestToken::Word(col_prefix, term, is_prefix)) => {
                self.advance();
                let col = if let Some(ref p) = col_prefix {
                    self.resolve_column(p)
                } else {
                    self.n_col
                };
                Ok(Some(ExprTestNode::Phrase(col, vec![(term, is_prefix)])))
            }
            Some(ExprTestToken::QuotedPhrase(terms)) => {
                self.advance();
                Ok(Some(ExprTestNode::Phrase(self.n_col, terms)))
            }
            Some(ExprTestToken::LParen) => {
                self.advance();
                let node = self.parse_or()?;
                if let Some(ExprTestToken::RParen) = self.peek() {
                    self.advance();
                }
                Ok(node)
            }
...
fn flatten_and(node: &ExprTestNode) -> Vec<ExprTestNode> {
    match node {
        ExprTestNode::And(l, r) => {
            let mut res = flatten_and(l);
            res.extend(flatten_and(r));
            res
        }
        _ => vec![node.clone()],
    }
...
fn build_balanced_and(nodes: &[ExprTestNode]) -> ExprTestNode {
    ...
    if nodes.len() == 2 {
        return ExprTestNode::And(Box::new(nodes[0].clone()), Box::new(nodes[1].clone()));
    }
    let mid = (nodes.len() + 1) / 2;
    ExprTestNode::And(
        Box::new(build_balanced_and(&nodes[..mid])),
        Box::new(build_balanced_and(&nodes[mid..])),
    )
}
...
fn rebalance_tree(node: &ExprTestNode) -> ExprTestNode {
    match node {
        ExprTestNode::And(_, _) => {
            let flat: Vec<ExprTestNode> = flatten_and(node)
                .into_iter()
                .map(|n| rebalance_tree(&n))
                .collect();
            build_balanced_and(&flat)
        }
        ExprTestNode::Or(l, r) => {
            ExprTestNode::Or(Box::new(rebalance_tree(l)), Box::new(rebalance_tree(r)))
        }
        ExprTestNode::Not(l, r) => {
            ExprTestNode::Not(Box::new(rebalance_tree(l)), Box::new(rebalance_tree(r)))
        }
        ExprTestNode::Near(d, l, r) => {
            ExprTestNode::Near(*d, Box::new(rebalance_tree(l)), Box::new(rebalance_tree(r)))
        }
...
pub fn func_fts3_exprtest(args: &[Value]) -> Result<Value> {
    ...
    let columns: Vec<String> = args[2..]
        .iter()
        .map(|v| value_to_string(v).to_ascii_lowercase())
        .collect();
...
pub fn func_fts3_exprtest_rebalance(args: &[Value]) -> Result<Value> {
    ...
    match parser.parse()? {
        Some(node) => {
            let r = rebalance_tree(&node);
            Ok(Value::Text(r.to_string_repr()))
        }
        None => Err(Error::with_message(
            ErrorCode::Error,
            "Error parsing expression",
