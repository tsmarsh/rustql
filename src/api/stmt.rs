...
pub fn sqlite3_prepare_v2<'a>(
    ...
    let dqs_dml = conn.db_config.dqs_dml;
    ...
    let compile_result = {
        ...
        let mut attached_schema_guards = Vec::new();
        for db in conn.dbs.iter().skip(2) {
            if let Some(schema) = db.schema.as_ref().and_then(|s| s.read().ok()) {
                attached_schema_guards.push((db.name.clone(), schema));
            }
        }
        let attached_schemas: Vec<(String, &crate::schema::Schema)> = attached_schema_guards
            .iter()
            ...
            if !stmt.read_only && !skip_auto_txn {
                if let Some(conn_ptr) = stmt.conn_ptr {
                    ...
                    if conn.get_autocommit() {
                        // Commit all btrees (main, temp, and attached databases)
                        for db in conn.dbs.iter() {
